...
def get_algorithm(name, nThreads = 1, **kwargs):
    match name:
    ...
    # Unequal length data series
        ...
        case "aeon.KNeighborsTimeSeriesRegressor":
            return aeon_dist.KNeighborsTimeSeriesRegressor(n_jobs=nThreads)
        case "aeon.RDSTRegressor":
            return aeon_shapelet.RDSTRegressor(n_jobs=nThreads)
    # Multivariate
        case "aeon.CanonicalIntervalForestRegressor":
            return aeon_int.CanonicalIntervalForestRegressor(n_jobs=nThreads)
        case "aeon.DrCIFRegressor":
            return aeon_int.DrCIFRegressor(n_jobs=nThreads)
        case "aeon.FCNRegressor":
            return aeon_deep.FCNRegressor()
        case "aeon.FreshPRINCERegressor":
            return aeon_feature.FreshPRINCERegressor(n_jobs=nThreads)
        case "aeon.HydraRegressor":
            return aeon_conv.HydraRegressor(n_jobs=nThreads, **kwargs)
        ...
        case "aeon.IntervalForestRegressor":
            return aeon_int.IntervalForestRegressor(n_jobs=nThreads)
        ...
        case "aeon.MiniRocketRegressor":
            return aeon_conv.MiniRocketRegressor(n_jobs=nThreads, **kwargs)
        case "aeon.MultiRocketHydraRegressor":
            return aeon_conv.MultiRocketHydraRegressor(n_jobs=nThreads, **kwargs)
        case "aeon.MultiRocketRegressor":
            return aeon_conv.MultiRocketRegressor(n_jobs=nThreads, **kwargs)
        case "aeon.QUANTRegressor":
            return aeon_int.QUANTRegressor()
        case "aeon.RISTRegressor":
            return aeon_hybrid.RISTRegressor(n_jobs=nThreads)
        case "aeon.RandomIntervalRegressor":
            return aeon_int.RandomIntervalRegressor(n_jobs=nThreads)
        case "aeon.RandomIntervalSpectralEnsembleRegressor":
            return aeon_int.RandomIntervalSpectralEnsembleRegressor(n_jobs=nThreads)
        case "aeon.ResNetRegressor":
            return aeon_deep.ResNetRegressor()
        case "aeon.RocketRegressor":
            return aeon_conv.RocketRegressor(n_jobs=nThreads)
        case "aeon.SummaryRegressor":
            return aeon_feature.SummaryRegressor(n_jobs=nThreads)
        case "aeon.TSFreshRegressor":
            return aeon_feature.TSFreshRegressor(n_jobs=nThreads)
        case "aeon.TimeCNNRegressor":
            return aeon_deep.TimeCNNRegressor()
        case "aeon.TimeSeriesForestRegressor":
            return aeon_int.TimeSeriesForestRegressor(n_jobs=nThreads)
    # Univariate
    ...
    if with_iciness:
        for fieldPath in dataFields:
            for metric in epoch_utils.ICE_METRICS:
                data_dict[fieldPath + f"_{metric}"] = []
            ...
            if fieldName in data[group].attrs.keys():
                ...
                if with_iciness:
                    for attr, val in data[fieldPath].attrs.items():
                        if attr in epoch_utils.ICE_METRICS:
                            data_dict[fieldPath + f"_{attr}"].append(val)

            if with_iciness:
                if "/Energy" in data.groups:
                    data_dict["ICEmetric_energyTransfer"].append(data["Energy"].ICEmetric_energyTransfer)
...
def normalise_data(data: ArrayLike, scaler = None) -> tuple[np.ndarray, StandardScaler]:
    if isinstance(data, list):
        data = np.array(data)
    if len(data.shape) == 1:
...
def denormalise_data(data: ArrayLike, scaler) -> np.ndarray:
    if len(data.shape) == 1:
        data = data.reshape(-1, 1)
...
def plot_three_dimensions(input_1_index : int, input_2_index : int, gpModel : GPModel, rawInputData : list = None, rawOutputData : list = None, showModels = True, saveAnimation = False, noTitle = False):
    ...
    cbar = plt.colorbar(label=epoch_utils.fieldNameToText(gpModel.outputName))
    ...
    plt.xlabel(epoch_utils.fieldNameToText(gpModel.inputNames[input_1_index]))
    plt.ylabel(epoch_utils.fieldNameToText(gpModel.inputNames[input_2_index]))
    if not noTitle:
...
def display_matrix_plots(inputs : dict, normalisedInputs : np.ndarray, outputs: dict, normalisedOutputs : dict = {}, noTitle : bool = False):
    ...
    for outName, outData in outputs.items():
        ...
        labels = [epoch_utils.fieldNameToText(outName)] + [epoch_utils.fieldNameToText(i) for i in inputs.keys()]
...
def plot_models(gpModels : list, rawInputData : dict = None, rawOutputData : dict = None, showModels : bool = True, saveAnimation : bool = False, noTitle : bool = False):
    ...
    for model in gpModels:
        ...
        for i in range(len(model.inputNames)):
            inputName = epoch_utils.fieldNameToText(model.inputNames[i])
            ...
            plt.ylabel(epoch_utils.fieldNameToText(model.outputName))
            plt.legend()
            #plt.tight_layout()
            if not noTitle:
                plt.title(f"GP predictions for {epoch_utils.fieldNameToText(model.outputName)} ({model.kernelName} kernel), all other input dimensions fixed to mean value", wrap=True)
            plt.show()
