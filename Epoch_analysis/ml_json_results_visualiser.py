...
def plotAccuracyByFrequency(resultsFile : Path):
    with open(resultsFile, "r") as f:
        ...
        if unit == "gyros":
            plt.xlabel(r"Frequency bandwidth [$\Omega_{c,\alpha}$]")
            plt.fill_betweenx(y = [0, 1], x1= cottrell_min_gyros, x2 = cottrell_max_gyros, color = "black", alpha = 0.15, label = "JET range")
            plt.legend(loc="lower right")
        else:
            plt.xlabel("Frequency bandwidth [MHz]")
            plt.fill_betweenx(y = [0, 1], x1= 100.0, x2 = cottrell_freq, color = "black", alpha = 0.15, label = "JET range")
