...
def run_energy_analysis(
    ...
    background_ion_KE : xr.DataArray = dataset[f'Derived_Average_Particle_Energy_{backgroundSpeciesName}'].mean(dim = "x_space").load()
    backgroundIonKE_mean = background_ion_KE.data # J
    ...
    electron_KE : xr.DataArray = dataset['Derived_Average_Particle_Energy_electron'].mean(dim = "x_space").load()
    electronKE_mean = electron_KE.data # J
    ...
    for variable, deltaED in deltaEnergies.items():
        ...
        if "fastIon" in variable:
            ...
            for threshold, idx in threshold_indices.items():
                # Record
                attr_str = f"{threshold}_idx" 
                energyStats["fastIonMeanEnergyDensity"].setncattr(attr_str, idx if idx is not None else "None")
                attr_str = f"{threshold}_time"
                energyStats["fastIonMeanEnergyDensity"].setncattr(attr_str, timeCoords[idx] if idx is not None else "None")
                
                # Plot
                if idx is not None:
                    ax.scatter(timeCoords[idx], smoothPctData[idx], marker="x", s=100.0, label = threshold)
            # ax.plot(timeCoords[1:], dE_dt, linestyle="dotted", color="tab:cyan", label = r"d($E_\alpha$)/dt")
        ...
        if colour:
            ax.plot(timeCoords, percentageED, alpha=0.5, color = colour)
            ax.plot(timeCoords, smoothPctData, label=e_utils.SPECIES_NAME_MAP[variable], linestyle="--", color = colour)
        else:
            ax.plot(timeCoords, percentageED, alpha=0.5)
            ax.plot(timeCoords, smoothPctData, label=e_utils.SPECIES_NAME_MAP[variable], linestyle="--")
    ...
    # Shade regions of MDI relevance
    if threshold_indices["MCI_start"] is not None:
        end_point = timeCoords[-1] if threshold_indices["linear_saturation"] is None else timeCoords[threshold_indices["linear_saturation"]]  
        ax.axvspan(timeCoords[threshold_indices["MCI_start"]], end_point, color = "green", alpha = 0.1, label = "linear MCI growth")
    if threshold_indices["linear_saturation"] is not None and threshold_indices["nonlinear_restitution"] is not None:
        ax.axvspan(timeCoords[threshold_indices["linear_saturation"]], timeCoords[threshold_indices["nonlinear_restitution"]], color = "blue", alpha = 0.1, label = "alpha re-energisation")
    if threshold_indices["nonlinear_saturation"] is not None:
        ax.axvspan(timeCoords[threshold_indices["nonlinear_saturation"]], timeCoords[-1], color = "red", alpha = 0.1, label = "NL saturation")
...
def process_simulation_batch(
    ...
    """
    Processes a batch of simulations (new for 2026):
    ...
    - Saves growth rates and plasma characteristics to netCDF, along with dispersion data for summation of 
        spectra across angles and holistic energy analyses

    ### Parameters:
        directory: Path -- Directory of simulation outputs, containing "run_n" folders each with sdf output files and an input.deck
        field: list -- List of string fields to use for analysis, either EPOCH output fields such as ['Magnetic_Field_Bz', 'Electric_Field_Ex']
    ...
    # simulationDataFolder is expected to contain all angles of all simulations with all SDF files.
    # This is a less flexible version in favour of smarter integration of multiple simulation angles and MCI analysis.

    # Identify angles
    angle_folders = glob.glob(str(simulationDataFolder / "9?") + os.path.sep)
    if len(angle_folders) < 1:
        raise ValueError("Can't identify angle folders")
    angleVals = [Path(f).name for f in angle_folders]
    # Identify simulation numbers
    if runNumber is None:
        simulation_run_folders = glob.glob(str(Path(angle_folders[0]) / "run_0_1000000/run_0_10000/run_0_100" / "run_*") + os.path.sep)
        simulation_names = [Path(f).name for f in simulation_run_folders]
    else:
        simulation_run_folders = glob.glob(str(Path(angle_folders[0]) / "run_0_1000000/run_0_10000/run_0_100" / f"run_{runNumber}") + os.path.sep)
        simulation_names = [f"run_{runNumber}"]
    if len(simulation_run_folders) < 1:
        raise ValueError("Can't identify simulation folders")
    # Build dictionary of simulation paths with all angles
    angle_folders_dict = {a : Path(b) for a, b in zip(angleVals, angle_folders)}
    run_folders_dict = {sim_name : {} for sim_name in simulation_names}
    for angleVal, angleFolder in angle_folders_dict.items():
        simulation_run_folders = glob.glob(str(angleFolder / "run_0_1000000/run_0_10000/run_0_100" / "run_*") + os.path.sep)
        for sim_folder_path in simulation_run_folders:
            run_folders_dict[Path(sim_folder_path).name][angleVal] = Path(sim_folder_path)
    ...
    for sim_name, angles in run_folders_dict.items():
        ...
        # Process sims individually first
        for angle_name, sim_folder in angles.items():
            ...
            # Read input deck
            inputDeck = {}
            with open(str(sim_folder / "input.deck")) as id:
                inputDeck = epydeck.loads(id.read())
            
            singleSimStats_filepath = os.path.join(str(analysisOutputFolder), angle_name, "data", f"{sim_name}_{angle_name}_stats.nc")
            statsRoot = nc.Dataset(singleSimStats_filepath, "a", format="NETCDF4")

            ion_gyroperiod, alfven_velocity = e_utils.calculate_simulation_metadata(inputDeck, ds, statsRoot, fastIonSpecies, backgroundIonSpecies)
        ...
        _, _ = e_utils.calculate_simulation_metadata(inputDeck, angle_datasets["90"], statsRoot, spaceCoordinateName="x_space", fastSpecies=fastIonSpecies, bkgdSpecies=backgroundIonSpecies)
