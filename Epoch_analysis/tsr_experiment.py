...
def regress_scanFrequencies(
    ...
    for frequency_bandwidth in frequencyBandwidths:
        ...
        frequency_bandwidth = float((frequency_bandwidth * u.MHz).to(u.Hz).value) if frequencyUnit == 'MHz' else frequency_bandwidth
        inputData = []
        for field in inputSpectraNames:
    ...
    # Write results and all predictions
    ml_utils.write_ML_result_to_file(battery, resultsFilepath.parent / resultsFilepath.name.replace('.csv', '.json'))
    if len(allPredictionsRecord) > 0:
